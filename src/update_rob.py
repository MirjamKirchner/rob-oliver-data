...
PATH_TO_FINDING_PLACES = os.path.join(
    PATH_TO_DATA, "processed/catalogued_finding_places.csv"
)

# TODO refactor this script such that it can save to local-file system or amazon S3
# TODO refactor this script such that it can build a clear-ml data set or not
def _update_rob_file_system(
    path_to_raw: str,
    path_to_rob: str,
    path_to_rob_engineered: str,
    path_to_finding_places: str,
    latest_update: datetime,
) -> (RobScraper, RobHistorizer):
    """
...
def update_rob_clearml(
    ...
    if latest_update is None:
        latest_update = pd.read_csv(
            path_to_rob_engineered,
            parse_dates=["Einlieferungsdatum", "Erstellt_am", "Sys_aktualisiert_am"],
            date_parser=pd.to_datetime,
        )["Sys_aktualisiert_am"].max()
        latest_update = (
            pd.to_datetime("1990-04-30")
            if (latest_update != latest_update)
            else latest_update
        )
    print(latest_update)

    # Update local files
    _update_rob_file_system(
        path_to_raw,
        path_to_rob,
        path_to_rob_engineered,
